...
class HolidayEntitlementDto(BaseModel):
    ...
    def amount_must_be_less_than_a_year(self):
        ...
        if (
            self.counting_method is EntitlementCountingMethod.HOURS
            and self.amount > 8784
        ):
            raise ValueError("Amount must not exceed a year")
        return self
    ...
    def to_holiday_entitlement(self) -> HolidayEntitlement:
        return HolidayEntitlement(
            ...
            amount_exceptions=(
                [
                    exception.to_amount_exception()
                    for exception in self.amount_exceptions
                ]
                if self.amount_exceptions is not None
                else None
            ),
        )
